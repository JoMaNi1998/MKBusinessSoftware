...
export const useOfferItems = ({
  ...
  isEditing
}: UseOfferItemsParams): UseOfferItemsReturn => {
  ...
  const addService = useCallback((service: Service): void => {
  ...
  const updateItem = useCallback((itemId: string, updates: Partial<OfferItem>): void => {
    setOfferData(prev => ({
      ...prev,
  ...
  const setOffer = useCallback((data: OfferFormData): void => {
    setOfferData(data);
  }, []);

  // Einzelne Felder aktualisieren
  const updateOfferField = useCallback((field: string, value: any): void => {
    setOfferData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const updateConditions = useCallback((updates: Partial<OfferConditions>): void => {
    setOfferData(prev => ({
      ...prev,
  ...
  const updateTotals = useCallback((updates: Partial<OfferTotals>): void => {
    setOfferData(prev => ({
      ...prev,
