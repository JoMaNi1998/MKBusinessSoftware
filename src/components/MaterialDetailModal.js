...
import { FirebaseService, MaterialService } from '../services/firebaseService';
...
const getStockStatusText = (stock = 0, heatStock = 0, orderStatus) => {
  if (Number(stock) === 0) return 'Nicht verfügbar';
  if (orderStatus === 'bestellt') return 'Bestellt';
  if (Number(stock) <= Number(heatStock)) return 'Niedrig';
  return 'Auf Lager';
};

const getStockStatusColor = (stock = 0, heatStock = 0, orderStatus) => {
  if (Number(stock) === 0) return 'text-red-600 bg-red-50';
  if (orderStatus === 'bestellt') return 'text-blue-600 bg-blue-50';
  if (Number(stock) <= Number(heatStock)) return 'text-orange-600 bg-orange-50';
  return 'text-green-600 bg-green-50';
};

const formatPriceForView = (price) => {
...
const buildDescription = ({ manufacturer, type, categoryName }) => {
...
const useCategoriesAndSpecs = (isOpen) => {
  ...
  const reload = useCallback(async () => {
    try {
      ...
      (specsData || []).forEach((spec) => {
        const key = spec.categoryId; // kann id oder name sein
        if (!key) return;
        if (!map[key]) map[key] = [];
        map[key].push(spec);
      });
  ...
  return { categories, specsByCategoryKey, loadingMeta, reload };
};

const findCategoryById = (categories, categoryId) =>
  categories.find((c) => c.id === categoryId);

const getSpecsForCategory = (specsByCategoryKey, category) => {
  if (!category) return [];
  // Specs können unter id **oder** name hängen
  return (
...
const computeNextMaterialId = (materials) => {
  const regex = /^MAT-(\d+)$/;
...
const requiredError = (label) => `${label} ist erforderlich`;
...
const MaterialModal = ({
  ...
  const { categories, specsByCategoryKey, loadingMeta, reload } =
  ...
  useEffect(() => {
    ...
    if (isView) return; // Formular wird nicht genutzt

    if (material && (isEdit || isCreate)) {
      // Edit-Fall: vorhandene Werte übernehmen
      setFormData({
        ...
        ean: material.ean ?? '',
        excludeFromAutoOrder: !!material.excludeFromAutoOrder,
  ...
  }, [categories, formData.categoryId, isView, material?.categoryId]);

  const selectedCategorySpecs = useMemo(
    () => getSpecsForCategory(specsByCategoryKey, selectedCategory),
    [specsByCategoryKey, selectedCategory]
  );
  ...
  const handleInputChange = (e) => {
    ...
    // Fehler für dieses Feld zurücksetzen, wenn etwas eingetragen wurde
  ...
  const handleCategoryChange = (value) => {
    ...
    setFormData((prev) => ({
      ...prev,
      categoryId: value,
      specifications: {} // beim Wechsel zurücksetzen
    }));
  ...
  const handleAddNewCategory = async () => {
    ...
    try {
      ...
      const newDoc = await FirebaseService.addDocument('categories', categoryData);
      await reload();
  ...
  const validate = () => {
    ...
    // Bestand (Startbestand nur bei Create, Edit ist gesperrt)
    if (isCreate) {
    ...
    // Spezifikationen: per Default alle Pflicht, außer spec.required === false
    selectedCategorySpecs.forEach((spec) => {
      const isRequired = spec.required !== false; // Standard: Pflicht
  ...
  const doSave = async () => {
    ...
    // Material-ID (bei Create automatisch)
    ...
    const dataToSave = {
      ...formData,
      materialID,
      stock: isCreate ? (parseInt(formData.stock, 10) || 0) : (material?.stock ?? 0),
      heatStock: parseInt(formData.heatStock, 10) || 0,
      itemsPerUnit: parseInt(formData.itemsPerUnit, 10) || 1,
      orderQuantity: parseInt(formData.orderQuantity, 10) || 0,
      price: formData.price === '' ? '' : Number(formData.price), // Zahl oder leer
      updatedAt: new Date(),
    ...
    // Formular nach Save säubern (Create) & schließen
    if (isCreate) {
      setFormData(blankForm);
      setErrors({});
    }
...
/**
 * Kompatibilitäts-Wrapper – drop-in replacement für deinen bestehenden Code
 */
