...
interface BackupSolutionsConfig {
  description: string;
  quantity: number;
  recommendedBreaker?: string;
  recommendedCable?: string;
...
interface PVCablesConfig {
  description: string;
  quantity: number;
}

interface PotentialausgleichConfig {
  description: string;
  quantity: number;
}

interface GeneratoranschlusskastenConfig {
  description: string;
  quantity: number;
}

interface PVConfiguration {
  configurationVersion?: string;
  timestamp?: any;
  inverters?: InverterConfig[];
  modules?: ModulesConfig;
  wallbox?: WallboxConfig;
  backupSolutions?: BackupSolutionsConfig;
  pvCables?: PVCablesConfig;
  potentialausgleichHESUK?: PotentialausgleichConfig;
  generatoranschlusskasten?: GeneratoranschlusskastenConfig;
}
...
interface ConfigurationWrapper {
  ...
  createdAt?: any;
  // Allow for direct properties if pvConfiguration is not nested
  configurationVersion?: string;
  timestamp?: any;
  inverters?: InverterConfig[];
  modules?: ModulesConfig;
...
interface PVConfigurationSectionProps {
...
const PVConfigurationSection: React.FC<PVConfigurationSectionProps> = ({
  configurations,
  loading,
  deletingConfigId,
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {configurations.map((configWrapper, index) => {
            const config: PVConfiguration = configWrapper.pvConfiguration || configWrapper;
            ...
            return (
              <div key={index} className="bg-white border rounded-lg p-4">
                ...
                {/* Wechselrichter */}
                {config.inverters?.length && config.inverters.length > 0 && (
                  <ConfigSection title="Wechselrichter" icon={Settings}>
                    {config.inverters.map((inv, idx) => (
                      <div key={idx} className="bg-gray-50 rounded p-3">
                        ...
                        {inv.strings?.length && inv.strings.length > 0 && (
...
interface RecommendedComponentsProps {
  breaker?: string;
  cable?: string;
  rcd?: string;
}

const RecommendedComponents: React.FC<RecommendedComponentsProps> = ({ breaker, cable, rcd }) => (
