...
const Page5PVGeneratorPruefberichtSingle = React.memo(function Page5PVGeneratorPruefberichtSingle({
  ...
  const getInverterSpecValue = (field: string): string =>
    ((vdeData as VDEData)[`inverter${inverterIndex}_${field}`] as string) || '';
  const getModuleSpecValue = (specId: string): string => getModuleSpec(vdeData, specId);
  ...
  const generatorParamRows: GeneratorParamRow[] = useMemo(
    () => [
  ...
  const wrProtectionRows: ProtectionRow[] = useMemo(
    () => [
  ...
  const branchProtectionRows: ProtectionRow[] = useMemo(
    () => [
  ...
  const wiringRows: ProtectionRow[] = useMemo(
    () => [
  ...
  const measurementRows: MeasurementRow[] = useMemo(
    () => [
  ...
  const insulationRows: InsulationRow[] = useMemo(
    () => [
    ...
    >
      ...
      <div className="mb-6">
        <table className="w-full border-collapse border border-gray-400 text-xs">
          <tbody>
            <tr>
              <td className="border p-2 font-semibold bg-gray-100 w-1/3">
              ...
              <td className="border p-2" colSpan={2}>
                <input
                  ...
                  value={
                    ...
                    ((vdeData as VDEData)[`inverterBrandModel_${inverterIndex}`] as string) ||
                    ''
                  }
                  onChange={(e: ChangeEvent<HTMLInputElement>) =>
                    handleVdeDataChange(`inverterBrandModel_${inverterIndex}`, e.target.value)
                  }
            ...
            <tr>
              <td className="border p-2 font-semibold bg-gray-100">
                Wechselrichter Typenkennzeichnung
              </td>
              <td className="border p-2" colSpan={2}>
                <input
                  type="text"
                  ...
                  onChange={(e: ChangeEvent<HTMLInputElement>) =>
                    handleVdeDataChange(`inverterTypeDesignation_${inverterIndex}`, e.target.value)
                  }
                  className="w-full bg-transparent"
                />
              </td>
            ...
            <tr>
              <td className="border p-2 font-semibold bg-gray-100">
                Bestimmungsgemäße WR-Funktion
              </td>
              <td className="border p-2 text-center" colSpan={2}>
                <input
                  type="checkbox"
                  checked={
                    ((vdeData as VDEData)[`inverterProperFunction_${inverterIndex}`] as boolean) ??
                    true
                  }
                  onChange={(e: ChangeEvent<HTMLInputElement>) =>
                    handleVdeDataChange(`inverterProperFunction_${inverterIndex}`, e.target.checked)
                  }
                  className="mr-1"
                />
              </td>
        ...
        <table className="w-full border-collapse border border-gray-400 text-xs">
          <thead>
            <tr className="bg-gray-100">
              <th className="border p-1 text-left font-semibold w-32" colSpan={2}>
                Strang
              </th>
              {stringNames.map((name, i) => (
                <th key={i} className="border p-1 text-center w-15" title={name}>
                  {name}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            ...
            <tr>
              ...
              {stringNames.map((_, i) => {
                ...
                const fallbackCount =
                  inverterData?.strings?.[gIdx]?.moduleCount ??
                ...
                return (
                  <td key={i} className="border p-1">
                    <input
                      ...
                      value={
                        ((vdeData as VDEData)[
                          `stringCount_${inverterIndex}_${gIdx + 1}`
                        ] as string) ?? fallbackCount
                      }
                      onChange={(e: ChangeEvent<HTMLInputElement>) =>
                        handleVdeDataChange(
                          `stringCount_${inverterIndex}_${gIdx + 1}`,
            ...
            {generatorParamRows.map((row, ri) => (
              <tr key={row.field}>
                {ri === 0 && (
                  <td
                    className="border p-1 font-semibold bg-gray-50 w-20"
                    rowSpan={generatorParamRows.length}
                  >
                    PV-Generator-Parameter
                  </td>
                )}
                  ...
                  return (
                    <td key={i} className="border p-1">
                      <input
                        ...
                        value={
                          ((vdeData as VDEData)[
                            `${row.field}_${inverterIndex}_${gIdx + 1}`
                          ] as string) ?? getModuleSpecValue(row.spec)
                        }
                        onChange={(e: ChangeEvent<HTMLInputElement>) =>
                          handleVdeDataChange(
                            `${row.field}_${inverterIndex}_${gIdx + 1}`,
              ...
              <tr key={row.field}>
                {ri === 0 && (
                  <td
                    ...
                    rowSpan={wrProtectionRows.length}
                  >
                    Schutzeinrichtung (WR)
                  </td>
                )}
                  ...
                  return (
                    <td key={i} className="border p-1">
                      <input
                        ...
                        value={
                          ((vdeData as VDEData)[
                            `string${row.field}_${inverterIndex}_${gIdx + 1}`
                          ] as string) ?? getInverterSpecValue(row.field)
                        }
                        onChange={(e: ChangeEvent<HTMLInputElement>) =>
                          handleVdeDataChange(
                            `string${row.field}_${inverterIndex}_${gIdx + 1}`,
            ...
            {branchProtectionRows.map((row, ri) => (
              <tr key={row.field}>
                {ri === 0 && (
                  <td
                    className="border p-1 font-semibold bg-gray-50"
                    rowSpan={branchProtectionRows.length}
                  >
                    Schutzeinrichtung (Zweigsicherung)
                  </td>
                )}
            ...
            {wiringRows.map((row, ri) => (
              <tr key={row.field}>
                ...
                {stringNames.map((_, i) => {
                  ...
                  const value =
                    ((vdeData as VDEData)[stringKey] as string) ||
                    ((vdeData as VDEData)[inverterKey] as string) ||
                    getInverterSpecValue(row.field) ||
                    '';
                  ...
                  return (
                    <td key={i} className="border p-1">
                      <input
                        ...
                        value={value}
                        onChange={(e: ChangeEvent<HTMLInputElement>) =>
                          handleVdeDataChange(stringKey, e.target.value)
                        }
                        className="w-full bg-transparent text-center text-xs"
                      />
                    </td>
            ...
            {measurementRows.map((row, ri) => (
              <tr key={row.field}>
                {ri === 0 && (
                  <td
                    className="border p-1 font-semibold bg-gray-50"
                    rowSpan={measurementRows.length}
                  >
            ...
            {/* Polarität */}
            <tr>
              <td className="border p-1 font-semibold bg-gray-50" colSpan={2}>
                ...
                return (
                  <td key={i} className="border p-1">
                    <input
                      ...
                      value={
                        ((vdeData as VDEData)[
                          `stringPolarityControl_${inverterIndex}_${gIdx + 1}`
                        ] as string) ?? 'i.O'
                      }
                      onChange={(e: ChangeEvent<HTMLInputElement>) =>
                        handleVdeDataChange(
            ...
            {insulationRows.map((row, ri) => (
              <tr key={row.field}>
                ...
                {stringNames.map((_, i) => {
                  const gIdx = pageOffset + i;
                  const isOptional =
                    row.field === 'optionalPositiveElectrode' ||
                    row.field === 'optionalNegativeElectrode';
                  return (
                    <td key={i} className="border p-1">
                      {isOptional ? (
                        ...
                        <input
                          ...
                          value={
                            ((vdeData as VDEData)[
                              `string${row.field}_${inverterIndex}_${gIdx + 1}`
                            ] as string) ?? (row.field === 'testVoltage' ? '1000' : '')
                          }
                          onChange={(e: ChangeEvent<HTMLInputElement>) =>
                            handleVdeDataChange(
                              `string${row.field}_${inverterIndex}_${gIdx + 1}`,
...
const Page5_PVGeneratorPruefbericht = React.memo(function Page5_PVGeneratorPruefbericht({
  vdeData,
  handleVdeDataChange,
}: PageProps) {
  const inverters: InverterWithMeta[] = useMemo(() => {
    ...
    if (list.length === 0) {
      ...
      for (let i = 0; i < count; i++) {
        list.push({
          type:
            ((vdeData as VDEData)[`inverterType${i + 1}`] as string) ||
            vdeData.inverterType ||
            'Unbekannt',
