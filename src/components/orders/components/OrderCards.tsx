...
import { formatOrderPrice, getOrderStatusColor, getOrderStatusText } from '@utils/orderHelpers';
import type { OrderCardsProps } from '@app-types/components/order.types';
        ...
        >
          ...
          <div className="flex justify-between items-start gap-3">
            ...
            {visibleColumns.status && (
              <span className={`px-2 py-1 rounded-full text-xs font-medium flex-shrink-0 ${getOrderStatusColor(material)}`}>
            ...
            <div className="mt-2 flex flex-wrap gap-3 text-sm">
              {visibleColumns.stock && (
                <div>
                  ...
                  <span className={`ml-1 font-medium ${(material.stock || 0) < 0 ? 'text-red-600' : 'text-gray-900'}`}>
                    {material.stock}
                  </span>
                </div>
              )}
              ...
              {visibleColumns.price && (
                <div>
                  ...
                  <span className="ml-1 font-medium text-gray-900">{formatOrderPrice(material.price)}</span>
                </div>
              )}
            </div>
          )}
