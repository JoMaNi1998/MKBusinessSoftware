...
import { Package, Zap, Home, Cable, Wrench, Sun } from 'lucide-react';
...
interface PVDefaults {
  ...
  defaultPotentialausgleich?: string;
  defaultPotentialausgleichUKUK?: string;
  defaultBefestigungPotentialausgleichUKUK?: string;
  defaultBefestigungLeistungsoptimierer?: string;
  defaultDammstoffduebel?: string;
  defaultKabelkanalSchrauben?: string;
  defaultKabelkanalDuebel?: string;
  defaultPvGeraeteSchrauben?: string;
  defaultPvGeraeteDuebel?: string;
  defaultSchraubenRohrschelle?: string;
  defaultDuebelRohrschelle?: string;
  ...
  defaultRohrschelleStandard?: string;
  defaultRohrschelleGross?: string;
  defaultRohrschelleOutdoor?: string;
  defaultKabelmanagement?: string;
  defaultInstallationsrohr?: string;
  defaultFlexrohrStandard?: string;
  defaultFlexrohrGross?: string;
  defaultInstallationsrohrOutdoor?: string;
  defaultMuffeOutdoor?: string;
  defaultKabelkanalStandard?: string;
  defaultKabelkanalGross?: string;
  defaultKabel5x15?: string;
  defaultKabel5x25?: string;
  defaultKabel5x4?: string;
  defaultKabel5x6?: string;
  defaultKabel5x10?: string;
  defaultKabel5x16?: string;
  defaultAdernleitung10mm2Blau?: string;
  defaultAdernleitung10mm2Schwarz?: string;
  defaultAdernleitung10mm2GruenGelb?: string;
  defaultAdernleitung16mm2Blau?: string;
  defaultAdernleitung16mm2Schwarz?: string;
  defaultAdernleitung16mm2GruenGelb?: string;
  defaultPvKabel?: string;
  defaultErdungHES?: string;
  defaultPotentialausgleichHESUK?: string;
  defaultSchutzleiterPV?: string;
  defaultErdungStaberder?: string;
  defaultSicherung16A?: string;
  defaultSicherung20A?: string;
  defaultSicherung25A?: string;
  defaultSicherung32A?: string;
  defaultSicherung50A?: string;
  defaultSicherung63A?: string;
  defaultFehlerstromschutzschalterWallbox?: string;
  defaultPotentialausgleichsschiene?: string;
  defaultHauptleitungsabzweigklemme?: string;
  defaultRJ45Stecker?: string;
  defaultSammelschienenklemme?: string;
  defaultAbdeckstreifen?: string;
...
interface DefaultComponentsSettingsProps {
  pvDefaults: PVDefaults;
  handlePvDefaultChange: (key: string, value: string | null) => void;
  materials: Material[];
}

const DefaultComponentsSettings: React.FC<DefaultComponentsSettingsProps> = ({
  pvDefaults,
  handlePvDefaultChange,
  materials,
}) => {

  // Hilfsfunktion für Material-Select
  const MaterialSelect: React.FC<MaterialSelectProps> = ({ label, value, onChange, categoryIds, filterFn }) => {
    let filteredMaterials: Material[] = [];

    if (filterFn) {
      filteredMaterials = materials.filter(filterFn);
    } else if (Array.isArray(categoryIds)) {
      filteredMaterials = materials.filter(m => m.categoryId && categoryIds.includes(m.categoryId));
    } else {
        ...
        >
          ...
          {filteredMaterials.map(m => (
            <option key={m.id} value={m.id}>
              {m.bezeichnung || m.description || m.materialID}
            </option>
          ))}
        </select>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e: ChangeEvent<HTMLSelectElement>) => handlePvDefaultChange('defaultRoofType', e.target.value)}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
          >
            <option value="">Kein Standard</option>
