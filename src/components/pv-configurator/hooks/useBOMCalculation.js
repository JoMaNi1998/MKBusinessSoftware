...
import { CAT, SPEC } from '../constants';
import { parseNum, getSpec } from '../utils';

/**
 * Hook für die BOM-Berechnung
 * @param {Object} params - Parameter für die Berechnung
 * @returns {{ bom: Array, warnings: Array }}
 */
  ...
  return useMemo(() => {
    ...
    if (chosen.inverterCable && inverterCount > 0) {
      bom.push({
        ...
        quantity: inverterCount * defaults.defaultCableLength,
        description: materialsById.get(chosen.inverterCable)?.description || `Mantelleitung (Wechselrichter) - ${defaults.defaultCableLength}m pro WR`,
        category: 'Kabel',
        isConfigured: true,
      });
    }
    ...
    if (chosen.wallboxCable && configuration.wallboxQty > 0) {
      bom.push({
        ...
        quantity: configuration.wallboxQty * defaults.defaultCableLength,
        description: materialsById.get(chosen.wallboxCable)?.description || `Mantelleitung (Wallbox) - ${defaults.defaultCableLength}m pro Wallbox`,
        category: 'Kabel',
        isConfigured: true,
      });
    }
    ...
    if (chosen.backupCable && configuration.notstromloesungenQty > 0) {
      bom.push({
        ...
        quantity: configuration.notstromloesungenQty * defaults.defaultCableLength,
        description: materialsById.get(chosen.backupCable)?.description || `Mantelleitung (Notstrom) - ${defaults.defaultCableLength}m pro Gerät`,
        category: 'Kabel',
        isConfigured: true,
      });
    }
