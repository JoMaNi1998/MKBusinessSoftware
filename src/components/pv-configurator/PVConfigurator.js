...
import { ChevronLeft, ChevronRight, CheckCircle, Save, Users, Grid, Layers, Zap, Battery, Plug } from 'lucide-react';
...
const PVConfigurator = () => {
  ...
  const bomInitializedForStep6 = useRef(false);
  // Ref to prevent project reset during initial load in edit mode
  const skipProjectResetRef = useRef(isEditMode);

  // Materialien indexieren
  const materialsById = useMemo(() => new Map(materials.map(m => [m.id, m])), [materials]);

  const optionsFromCategory = useCallback(
    (catId) => materials.filter(m => m.categoryId === catId).map(m => ({ value: m.id, label: m.description || m.name || m.id })),
    [materials]
  );
  ...
  const { recommendations, chosen, overrideRec, setOverrideRec } = useRecommendations({ configuration, materialsById, pvDefaults });

  // BOM Calculation Hook
  const baseBOMWithWarnings = useBOMCalculation({ configuration, layoutTotals, materials, materialsById, chosen, defaults });
  ...
  useEffect(() => {
    ...
    setCustomerProjects(projects.filter(p => p.customerID === selectedCustomer));
  ...
  useEffect(() => {
    ...
    if (existingConfig) {
      ...
      if (existingConfig.configuration) setConfiguration(existingConfig.configuration);
      else if (existingConfig.configData) setConfiguration(existingConfig.configData);
      if (existingConfig.billOfMaterials?.length > 0) {
        setCalculatedBOM(existingConfig.billOfMaterials.map(item => ({ materialID: item.materialId, description: item.name, quantity: item.quantity, category: item.category, isConfigured: item.isConfigured, isManual: item.isManual })));
      }
    }
  ...
  // Validation
  const pairRequired = (matId, qty) => (!!matId && qty > 0) || (!matId && !qty);
  const validateStep = (step) => {
    ...
    if (step === 3) { configuration.inverters.forEach((inv, i) => { if (!inv.type) errors.push(`Wechselrichter ${i + 1}`); }); const ts = configuration.inverters.flatMap(inv => inv.strings || []).reduce((s, st) => s + (parseInt(st.modules) || 0), 0); if (layoutTotals.totalModules > 0 && layoutTotals.totalModules !== ts) errors.push('String-Module-Anzahl'); }
  ...
  // String/WR Helpers
  const addStringToInverter = (i) => setConfiguration(prev => { const next = { ...prev, inverters: prev.inverters.map(x => ({ ...x, strings: [...x.strings] })) }; next.inverters[i].strings.push({ name: `${i + 1}.${next.inverters[i].strings.length}`, modules: 1 }); return next; });
  const removeStringFromInverter = (i, sIdx) => setConfiguration(prev => { const next = { ...prev, inverters: prev.inverters.map(x => ({ ...x, strings: [...x.strings] })) }; next.inverters[i].strings.splice(sIdx, 1); next.inverters[i].strings = next.inverters[i].strings.map((s, idx) => ({ ...s, name: `${i + 1}.${idx}` })); return next; });
  const updateStringModules = (i, sIdx, val) => { const n = Math.max(1, parseInt(val) || 1); setConfiguration(prev => { const next = { ...prev, inverters: prev.inverters.map(x => ({ ...x, strings: [...x.strings] })) }; next.inverters[i].strings[sIdx].modules = n; return next; }); };
  ...
  const removeInverter = (i) => setConfiguration(prev => ({ ...prev, inverters: prev.inverters.filter((_, idx) => idx !== i) }));
  ...
  const addMaterialToBOM = (materialId, quantity = 1, isManual = false) => {
    ...
    else { setCalculatedBOM(prev => [...prev, { materialID: materialId, description: material.description, quantity, isManual }]); }
    if (isManual) setShowAddMaterialModal(false);
  };

  // Speichern
  const handleSaveConfiguration = async (status = CONFIG_STATUS.DRAFT) => {
    setIsSaving(true);
    try {
      const selectedMod = materialsById.get(configuration.module);
      const powerWatt = parseNum(selectedMod?.specifications?.kvjYqZiEZiB5kCeDQRzx || 0);
      ...
      const configData = {
        ...
        billOfMaterials: calculatedBOM.map(item => ({
          ...
          name: materialsById.get(item.materialID)?.artikelname || materialsById.get(item.materialID)?.description || item.description,
          articleNumber: materialsById.get(item.materialID)?.artikelnummer || '',
        ...
        totals: {
          ...
          batteryKwh: configuration.batteryKwh || 0
        }
      };
      ...
      if (result?.success) {
        showNotification(isEditMode ? 'Konfiguration aktualisiert' : 'Konfiguration gespeichert', 'success');
  ...
  const handleBookToCustomer = async () => {
    if (!selectedCustomer || !selectedProject || calculatedBOM.length === 0) { showNotification('Bitte Kunde und Projekt auswählen und Konfiguration vervollständigen!', 'warning'); return; }
    if (isBooking) return;
    setIsBooking(true);
    try {
      ...
      const getNextVersion = () => { if (projectConfigs.length === 0) return '1.0'; const versions = projectConfigs.map(c => parseFloat(c.pvConfiguration?.configurationVersion || '1.0')).filter(v => !isNaN(v)); return (Math.max(...versions, 0) + 1.0).toFixed(1); };
      const nextVersion = getNextVersion();
      const bookingId = `pv-booking-${selectedProject}-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`;
      const bookingData = {
        id: bookingId, customerID: selectedCustomer, customerName: customers.find(c => c.id === selectedCustomer)?.firmennameKundenname || 'Unbekannt',
        projectID: selectedProject, projectName: projects.find(p => p.id === selectedProject)?.projectName || 'Unbekannt', type: 'Ausgang',
        materials: calculatedBOM.map(i => { const material = materials.find(m => m.materialID === i.materialID || m.id === i.materialID); const priceAtBooking = material?.price || 0; return { materialID: i.materialID, quantity: i.quantity, description: i.description, priceAtBooking, totalCost: priceAtBooking * i.quantity, isConfigured: i.isConfigured || false, category: i.category || '' }; }),
      ...
      for (const item of calculatedBOM) { const material = materials.find(m => m.materialID === item.materialID || m.id === item.materialID); if (material) await updateMaterialStock(material.id, -item.quantity); }
    ...
    } catch (e) { console.error('Fehler beim Buchen:', e); showNotification('Fehler beim Buchen der Stückliste!', 'error'); }
  ...
  return (
    <div className="h-dvh flex flex-col bg-gray-50">
      ...
      {showAddMaterialModal && <AddMaterialModal materials={materials} onAddMaterial={(materialId, quantity) => addMaterialToBOM(materialId, quantity, true)} onClose={() => setShowAddMaterialModal(false)} />}
    </div>
  );
};
