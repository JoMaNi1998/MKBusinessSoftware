...
const ConfigPreview = ({ config, isOpen, onClose, onEdit, onBook }) => {
  const navigate = useNavigate();
  const { customers } = useCustomers();
  const { projects } = useProjects();
  const { materials } = useMaterials();

  // Hilfsfunktion um Material-Map zu erstellen
  ...
  const customer = useMemo(() => {
    return customers.find(c => c.id === config?.customerID);
  ...
  // Konfiguration - unterstützt beide Feldnamen
  const configuration = config?.configuration || config?.configData || {};
  ...
  // BOM Tabelle rendern
  const renderBOMTable = (items, title, bgColor, borderColor) => {
            ...
            {items.map((item, index) => (
              <tr key={index}>
                <td className="px-4 py-2 whitespace-nowrap text-sm">
                  ...
                  {item.articleNumber && (
                    <div className="text-xs text-gray-500">{item.articleNumber}</div>
                  )}
                </td>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-gray-100">
          <div className="max-w-3xl mx-auto space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900">
                      {project?.projektname || project?.name || '-'}
                    </p>
            ...
            <div className="bg-white shadow rounded-lg p-6">
              ...
              {config.billOfMaterials?.length > 0 ? (
