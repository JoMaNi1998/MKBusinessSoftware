...
const OfferPDFPreview = ({ offer, isOpen, onClose, onEdit }) => {
  ...
  // Helper-Funktion: Text in Seiten aufteilen (ca. 2800 Zeichen pro Seite)
  const splitTextIntoPages = (text, maxCharsPerPage = 2800) => {
    ...
    for (const paragraph of paragraphs) {
      // Wenn Absatz allein zu lang ist, nach Zeilen aufteilen
      if (paragraph.length > maxCharsPerPage) {
      ...
      } else if ((currentPage + '\n\n' + paragraph).length > maxCharsPerPage) {
        // Neue Seite beginnen
        if (currentPage) {
          pages.push(currentPage.trim());
        }
        ...
        // Zum aktuellen Seiteninhalt hinzufügen
        currentPage = currentPage ? currentPage + '\n\n' + paragraph : paragraph;
      }
    }
  ...
  const renderPageContent = () => (
    <>
          ...
          <div className="page-body">
            ...
            <div className="flex justify-between items-start mb-2">
              <div>
                ...
                {offer.pvConfigFiles?.filter(file => file.type?.startsWith('image/')).length > 0 && (
                  <img
                    src={offer.pvConfigFiles.filter(file => file.type?.startsWith('image/'))[0]?.url}
          ...
          <div className="mb-8">
            <table className="w-full text-sm">
              ...
              <tfoot className="footer-spacer">
                <tr><td colSpan="6" style={{height: '20mm'}}></td></tr>
              </tfoot>
            </table>
          </div>
      ...
      {additionalPages && additionalPages.length > 0 && additionalPages.map((page, pageIndex) => (
