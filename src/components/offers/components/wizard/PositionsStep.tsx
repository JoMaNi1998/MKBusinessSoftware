...
import type { OfferFormData, OfferItem, OfferConditions, OfferTotals } from '@app-types/components/offer.types';
...
interface PositionsStepProps {
  ...
  onDeleteFile: (file: PVConfigFile) => void;
  onUpdateItem: (itemId: string, updates: Partial<OfferItem>) => void;
  onRemoveItem: (itemId: string) => void;
  onAddManualItem: () => void;
  onUpdateOfferField: (field: string, value: any) => void;
  onUpdateConditions: (updates: Partial<OfferConditions>) => void;
  onUpdateTotals: (updates: Partial<OfferTotals>) => void;
  onGoToServices: () => void;
}

const PositionsStep: React.FC<PositionsStepProps> = ({
  offerData,
      ...
      <div className="max-w-[210mm] mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
        <div className="p-8">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex items-center justify-end gap-2">
                  ...
                  <input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => onUpdateOfferField('offerDate', e.target.value)}
                ...
                <div className="flex items-center justify-end gap-2">
                  ...
                  <input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => onUpdateConditions({ validUntil: e.target.value })}
                    className="border border-gray-300 rounded px-2 py-1 text-sm"
                  />
                </div>
              </div>
                  ...
                  <tr key={item.id || index} className="border-b border-gray-100 group">
                    ...
                    <td className="py-3">
                      <input
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => onUpdateItem(item.id, { shortText: e.target.value })}
                        className="w-full bg-transparent border-0 p-0 focus:ring-0 font-medium text-gray-900"
                      />
                      {item.type === 'custom' ? (
                        <textarea
                          value={item.longText || ''}
                          onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => onUpdateItem(item.id, { longText: e.target.value })}
                      ...
                      {item.laborFactor && item.laborFactor > 1 && (
                    ...
                    <td className="py-3 align-top">
                      <input
                        ...
                        step="0.1"
                        value={item.quantity}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => onUpdateItem(item.id, { quantity: parseFloat(e.target.value) || 0 })}
                    ...
                    <td className="py-3 align-top">
                      <input
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => onUpdateItem(item.id, { unitPriceNet: parseFloat(e.target.value) || 0 })}
                        className="w-full text-right bg-transparent border-0 p-0 focus:ring-0"
                      />
                    </td>
                    <td className="py-3 align-top">
                      <div className="flex items-center justify-end">
                        <input
                          ...
                          max="100"
                          value={item.discount}
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) => onUpdateItem(item.id, { discount: parseFloat(e.target.value) || 0 })}
                          className="w-12 text-right bg-transparent border-0 p-0 focus:ring-0"
                        />
                        <span className="text-gray-400 ml-0.5">%</span>
                      </div>
                    </td>
                    <td className="py-3 text-right font-medium align-top">{formatCurrency(item.totalNet)}</td>
                    <td className="py-3 align-top">
                      <button
          ...
          <div className="flex justify-end mb-8">
            <div className="w-72">
              {/* Mengenstaffel Info */}
              {(offerData.totals as any)?.quantityScaleDiscount > 0 && (
                <div className="bg-green-50 text-green-700 rounded p-2 mb-3 text-xs">
                  <div className="flex items-center">
                    <TrendingDown className="h-3 w-3 mr-1" />
                    Mengenstaffel: {(offerData.totals as any).moduleCount} Module ({(offerData.totals as any).quantityScaleTier?.label})
                  </div>
                  <div className="flex justify-between mt-1">
                    <span>-{(offerData.totals as any).quantityScaleDiscount}% auf Arbeitszeit</span>
                    <span>-{formatCurrency((offerData.totals as any).laborReductionTotal)}</span>
                  </div>
                </div>
              )}
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => onUpdateTotals({ discountPercent: parseFloat(e.target.value) || 0 })}
                      className="w-12 text-right border border-gray-200 rounded px-1 py-0.5 text-xs"
                    />
                    <span className="text-gray-400 ml-1">%</span>
                    {(offerData.totals?.discountAmount ?? 0) > 0 && (
                      <span className="ml-2 text-red-600">-{formatCurrency(offerData.totals?.discountAmount)}</span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => onUpdateTotals({ taxRate: parseFloat(e.target.value) || 0 })}
                      className="w-12 text-right border border-gray-200 rounded px-1 py-0.5 text-xs"
                    />
                    <span className="text-gray-400 ml-1">%</span>
                    <span className="ml-2">{formatCurrency(offerData.totals?.taxAmount)}</span>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center mt-3 pt-3 border-t border-dashed">
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => onUpdateOfferField('depositPercent', parseFloat(e.target.value) || 0)}
                      className="w-12 text-right border border-gray-200 rounded px-1 py-0.5 text-xs"
                    />
                    <span className="text-gray-400 ml-1">%</span>
                    <span className="ml-2 font-medium">{formatCurrency((offerData.totals?.grossTotal || 0) * ((offerData.depositPercent ?? 50) / 100))}</span>
                  </div>
                </div>
              </div>
            </div>
