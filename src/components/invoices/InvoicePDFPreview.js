...
const InvoicePDFPreview = ({ invoice, isOpen, onClose, onEdit }) => {
  ...
  // Print-Version des Rechnungsinhalts - alles in einer Tabelle für korrekten Footer-Spacer
                  ...
                  <div className="flex justify-end mb-8 summary-section">
                    <div className="w-72">
                      <div className="space-y-2 text-sm">
                        ...
                        {invoice.invoiceType === INVOICE_TYPE.DEPOSIT && invoice.totals?.finalAmount && (
                          <>
                            <div className="border-t my-2"></div>
                            <div className="flex justify-between border-t-2 border-gray-900 pt-2 text-lg font-bold">
                              <span>Anzahlung ({invoice.totals?.invoicePercent || invoice.offerDepositPercent || 50}%):</span>
                              <span>{formatPrice(invoice.totals?.finalAmount)}</span>
                            </div>
                          </>
                        )}

                        {/* Bei Schlussrechnung: Gesamtübersicht zeigen */}
                        {invoice.invoiceType === INVOICE_TYPE.FINAL && (
                          <>
                            <div className="border-t my-2"></div>
                            <div className="flex justify-between text-green-600">
                              <span>Abzgl. Anzahlung ({invoice.offerDepositPercent || 50}%):</span>
                              <span>- {formatPrice(invoice.totals?.depositAmount || invoice.depositAmount || (invoice.totals?.grossTotal * (invoice.offerDepositPercent || 50) / 100))}</span>
                            </div>
                            <div className="flex justify-between border-t-2 border-gray-900 pt-2 text-lg font-bold">
                              <span>Zu zahlender Restbetrag:</span>
                              <span>{formatPrice(invoice.totals?.finalAmount || invoice.totals?.grossTotal)}</span>
                            </div>
                          </>
                        )}
