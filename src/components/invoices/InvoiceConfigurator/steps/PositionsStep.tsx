...
import type { InvoicePositionsStepProps } from '@app-types/components/invoice.types';

const PositionsStep: React.FC<InvoicePositionsStepProps> = ({
  invoiceData,
  setInvoiceData,
  handleUpdateItem,
  handleRemoveItem,
        ...
        <div className="p-8">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between items-center">
                  ...
                  <input
                    ...
                    onChange={(e: ChangeEvent<HTMLInputElement>) => setInvoiceData(prev => ({ ...prev, invoiceDate: e.target.value }))}
                ...
                <div className="flex justify-between items-center">
                  ...
                  <input
                    ...
                    onChange={(e: ChangeEvent<HTMLInputElement>) => setInvoiceData(prev => ({ ...prev, dueDate: e.target.value }))}
                    className="border border-gray-200 rounded px-2 py-1 text-sm focus:ring-1 focus:ring-blue-500"
                  />
                </div>
              </div>
                  ...
                  <tr key={item.id || index} className="border-b border-gray-100 group">
                    ...
                    <td className="py-3">
                      <input
                        ...
                        onChange={(e: ChangeEvent<HTMLInputElement>) => handleUpdateItem(item.id, { shortText: e.target.value })}
                        className="w-full bg-transparent border-0 p-0 focus:ring-0 font-medium text-gray-900"
                      />
                      {item.type === 'manual' ? (
                        <textarea
                          value={item.longText || ''}
                          onChange={(e: ChangeEvent<HTMLTextAreaElement>) => handleUpdateItem(item.id, { longText: e.target.value })}
                    ...
                    <td className="py-3 align-top">
                      <input
                        ...
                        step="0.1"
                        value={item.quantity}
                        onChange={(e: ChangeEvent<HTMLInputElement>) => handleUpdateItem(item.id, { quantity: parseFloat(e.target.value) || 0 })}
                    ...
                    <td className="py-3 align-top">
                      <input
                        ...
                        onChange={(e: ChangeEvent<HTMLInputElement>) => handleUpdateItem(item.id, { unitPriceNet: parseFloat(e.target.value) || 0 })}
                        className="w-full text-right bg-transparent border-0 p-0 focus:ring-0"
                      />
                    </td>
                    <td className="py-3 align-top">
                      <div className="flex items-center justify-end">
                        <input
                          ...
                          onChange={(e: ChangeEvent<HTMLInputElement>) => handleUpdateItem(item.id, { discount: parseFloat(e.target.value) || 0 })}
          ...
          <div className="flex justify-end mb-8">
            <InvoiceSummary
              ...
              editable={true}
              onDiscountChange={(value: number) => setInvoiceData(prev => ({
                ...prev,
                totals: { ...prev.totals, discountPercent: value }
              }))}
              onTaxRateChange={(value: number) => setInvoiceData(prev => ({
                ...prev,
