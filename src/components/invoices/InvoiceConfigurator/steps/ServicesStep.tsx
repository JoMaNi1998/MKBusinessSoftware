...
import type { InvoiceServicesStepProps } from '@app-types/components/invoice.types';
import type { ServiceCategoryConfig } from '@app-types/contexts/serviceCatalog.types';

const LABOR_FACTOR_LABELS: Record<string, string> = {
  dach: 'Dach',
  elektro: 'Elektro',
...
const ServicesStep: React.FC<InvoiceServicesStepProps> = ({
  ...
  const getCategoryIcon = (iconName?: string): ReactNode => {
    if (!iconName) return <Package className="h-5 w-5" />;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {(['dach', 'elektro', 'geruest'] as const).map(factorType => {
            ...
            return (
              <div key={factorType}>
                ...
                <select
                  ...
                  onChange={(e: ChangeEvent<HTMLSelectElement>) => setLaborFactorSelections(prev => ({
                    ...prev,
            ...
            return (
              <div key={category.id} className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e: ChangeEvent<HTMLSelectElement>) => handleServiceSelection(category.id, e.target.value)}
                ...
                {selectedService?.isPackage && selectedService?.subItems?.length && selectedService.subItems.length > 0 && (
                ...
                {selectedService?.replaces?.length && selectedService.replaces.length > 0 && (
      ...
      <div className="border rounded-lg overflow-hidden">
        ...
        <div className="px-4 py-3 border-b">
          <div className="relative">
            ...
            <input
              ...
              onChange={(e: ChangeEvent<HTMLInputElement>) => setServiceSearchTerm(e.target.value)}
        ...
        {serviceSearchTerm.trim() && (
          <div className="divide-y divide-gray-100">
            {(SERVICE_CATEGORIES as ServiceCategoryConfig[]).map(category => {
